            ...
            default:
                ...
                if (FD_ISSET(sock, &ready)) {
                    ...
                    if ((acc = accept(sock, (struct sockaddr *) &from, &len)) == -1) {
                        ...
                        if (pos != -1) {
                            child[pos] = acc; // クライアントソケットを配列に追加
                            current_socket_count++; // 現在のソケット数を更新
                            if (current_socket_count > max_socket_count) {
                                max_socket_count = current_socket_count; // 最大のソケット数を更新
                            }
                        }
                    }
                }
                ...
                for (i = 0; i < childNum; i++) {
                    if (child[i] != -1 && FD_ISSET(child[i], &ready)) {
                        ...
                        if (echoback(child[i]) == false) {
                            close(child[i]);
                            child[i] = -1;
                            current_socket_count--; // ソケットが閉じられたので、現在のソケット数を減らす
                        }
                    }
                }
        }
